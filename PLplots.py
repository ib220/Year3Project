...
for m in range(1,13): 
     ...
     Tropopause = concen_month(tropo,m)
...
for z in range(1,13): 
    data_months = concen_month(CO14_prod,z)
    #saving data as nc file
    ...
    #defining the tropopause level - only works for pressure plot 
    Tropopause = concen_month(tropo,z)    
    trop_level = 1000 * np.exp( -Tropopause.data/ 8.5e3) # converting tropopause from m to hBar

    #using first order approximation to find pressure 
    pressure = 1000 * np.exp( -altitude / 8.5e3)
    #plt.pcolor(latitude,altitude,data_months.data) # plotting latitude 
    prod_new = (data_months.data*prod_conv)*6.02e23*2.628e6 # multiplied by avogardos for molecules 
    #multipled by number of seconds in a month to get molecules/cm3/months
    plt.pcolor(latitude,pressure,prod_new) #plotting as function of pressure
    plt.plot(latitude,trop_level, color='red', label='Tropopause') # plot ropopause 
    ...
    plt.title('14CO Production in month #{}'.format(z))
    plt.xlabel('Latitude')
    plt.ylabel('Pressure (hbar)')
    #plt.ylabel('Altitude (km)')
    plt.legend(loc='lower left')
    plt.savefig('ProdPlot/14C_prod{}.png'.format(z))
    plt.show()


#%%#plotting 14CO loss for each month and saving data
for y in range(1,13): 
    data_months = concen_month(CO14_loss,y)
    #saving data as nc file
    iris.save(data_months,'LossData/14COloss{}.nc'.format(y) )
    ...
    #defining the tropopause level - only works for pressure plot 
    Tropopause = concen_month(tropo,y)    
    trop_level = 1000 * np.exp( -Tropopause.data/ 8.5e3) # converting tropopause from m to hBar
    ...
    plt.title('14CO Loss in month #{}'.format(y))
    plt.xlabel('Latitude')
    plt.ylabel('Pressure (hbar)')
    #plt.ylabel('Altitude (km)')
    plt.legend(loc='lower left')
    plt.savefig('LossPlot/14C_loss_nonfix{}.png'.format(y))
...
for i in range(1,13):
    ...
    pressure = 1000 * np.exp( -altitude / 8.5e3)
    #plt.pcolor(latitude,altitude,data_months.data) # plotting latitude
    plt.contourf(latitude,pressure,data_months,cmap='seismic',vmax=1,vmin=-1) #plotting as function of pressure
    ...
    plt.title('(P-L) in 14CO in month #{}'.format(i))
    plt.xlabel('Latitude')
    
    plt.ylabel('Pressure (hbar)')
    #plt.ylabel('Altitude (km)')
    plt.legend(loc='lower left')
    plt.savefig('NetPlot/14C_net_cont{}.png'.format(i))
    plt.show()
...
#%%
rprod =  iris.load('ProdData/14COprod.nc')[0]
rloss = iris.load_cube('LossData/14COloss1.nc')
rloss.units = 'kg m-2 s-1'
#print(  rloss)
data_month = (((rprod.data*prod_conv  - rloss.data*loss_conv))*6.02e23)*2.628e6
data_months = data_month #* month_topS
#extracting latitude at mindpoint of each cell 
latitude = rloss.coord('latitude').points
#extracting altitude at mindpoint of each cell  in m 
altitude = (rloss.coord('level_height').points)
#defining the tropopause level - only works for pressure plot 
Tropopause = concen_month(tropo,1)    
trop_level = 1000 * np.exp( -Tropopause.data/ 8.5e3) # converting tropopause from m to hBar    
